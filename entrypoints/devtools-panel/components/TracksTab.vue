...
const trackStatsByAlias = computed(() => {
  ...
  for (const stream of props.session.streams.values()) {
    ...
    if (!stats) {
      stats = { totalBytes: 0, firstDataAt: undefined, lastDataAt: undefined, streamCount: 0, dgGroupCount: 0 };
      map.set(alias, stats);
    }
...
const trackRows = computed((): TrackRow[] => {
  ...
  return filteredTracks.value.map((track) => {
    ...
    let bitrate = 0;
    if (stats && stats.totalBytes > 0 && stats.firstDataAt != null && stats.lastDataAt != null) {
      const activeSec = Math.max((stats.lastDataAt - stats.firstDataAt) / 1000, 1);
...
const groups = computed((): NsGroup[] => {
  ...
  for (const row of trackRows.value) {
    const ns = row.track.trackNamespace.length > 0 ? row.track.trackNamespace.join('/') : '(no namespace)';
    let g = map.get(ns);
    if (!g) {
      g = { ns, rows: [] };
      map.set(ns, g);
...
function formatDuration(track: TrackEntry, stats: TrackStats | undefined): string {
  const start = stats?.firstDataAt ?? track.subscribedAt;
  if (start == null) return '\u2014';
  const end = (track.status === 'done' || track.status === 'error')
    ? (track.subscribeDoneAt ?? track.subscribeErrorAt ?? stats?.lastDataAt ?? start)
    : Date.now();
...
<template>
  <div class="tracks-tab">
    ...
    <div v-if="filteredTracks.length === 0" class="empty-hint">
      {{ allTracks.length === 0 ? 'No track subscriptions yet' : 'No tracks match current filters' }}
    </div>
    ...
    <div v-else class="tracks-table-wrap">
      <table class="tracks-table">
        ...
        <tbody>
          ...
          <template v-if="groupByNs">
            <template v-for="g in groups" :key="g.ns">
              ...
              <template v-if="!collapsedGroups.has(g.ns)">
                <tr v-for="row in g.rows" :key="row.track.subscribeId" class="track-row">
                  <td class="col-color"><span class="color-dot" :style="{ background: trackColor(row.track) }" /></td>
                  <td class="col-dir" :class="row.track.direction === 'tx' ? 'direction-tx' : 'direction-rx'">
                    {{ row.track.direction === 'tx' ? '\u2191' : '\u2193' }}
                  </td>
                  <td class="col-status" :class="`status-${row.track.status}`">{{ statusIcon(row.track.status) }}</td>
                  <td class="col-name mono" :title="row.track.fullName">{{ row.track.trackName }}</td>
                  <td class="col-time mono">{{ formatTime(row.track.subscribedAt) }}</td>
                  <td class="col-time mono">{{ formatTime(row.track.subscribeOkAt) }}</td>
                  <td class="col-latency mono">{{ formatLatency(row.track) }}</td>
                  <td class="col-time mono">{{ formatTime(row.stats?.firstDataAt) }}</td>
                  <td class="col-bytes mono">{{ row.stats ? formatBytes(row.stats.totalBytes) : '\u2014' }}</td>
                  <td class="col-bitrate mono">{{ formatBitrate(row.bitrate) }}</td>
                  <td class="col-duration mono">{{ formatDuration(row.track, row.stats) }}</td>
                  <td class="col-error" :title="row.track.errorReason ?? ''">{{ row.track.errorReason ?? '' }}</td>
                </tr>
              </template>
            </template>
          </template>
          ...
          <!-- Flat view -->
          <template v-else>
            <tr v-for="row in trackRows" :key="row.track.subscribeId" class="track-row">
              <td class="col-color"><span class="color-dot" :style="{ background: trackColor(row.track) }" /></td>
              <td class="col-dir" :class="row.track.direction === 'tx' ? 'direction-tx' : 'direction-rx'">
                {{ row.track.direction === 'tx' ? '\u2191' : '\u2193' }}
              </td>
              ...
              <td class="col-name mono" :title="row.track.fullName">
                <span v-if="row.track.trackNamespace.length > 0" class="track-ns">{{ row.track.trackNamespace.join('/') }}/</span>{{ row.track.trackName }}
              </td>
              <td class="col-time mono">{{ formatTime(row.track.subscribedAt) }}</td>
              <td class="col-time mono">{{ formatTime(row.track.subscribeOkAt) }}</td>
              <td class="col-latency mono">{{ formatLatency(row.track) }}</td>
...
.col-color { width: 16px; text-align: center; }
.col-dir { width: 24px; text-align: center; font-weight: bold; }
.col-status { width: 24px; text-align: center; }
.col-name { max-width: 200px; }
.col-time { width: 90px; }
.col-latency { width: 60px; }
.col-bytes { width: 70px; text-align: right; }
.col-bitrate { width: 80px; text-align: right; }
.col-duration { width: 65px; text-align: right; }
.col-error { max-width: 120px; color: var(--text-error); font-size: 10px; }
