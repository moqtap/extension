...
const selectedSession = computed(() =>
  selectedSessionId.value ? sessions.value.get(selectedSessionId.value) ?? null : null,
);
...
function compactTooltip(s: SessionEntry): string {
  const proto = s.protocol === 'moqt' ? `MoQT d${s.draft}` : s.protocol === 'detecting' ? 'Detecting...' : 'WT';
  const status = s.imported ? 'imported' : s.closed ? 'closed' : 'open';
  let url: string;
  try { url = new URL(s.url).hostname; } catch { url = s.url; }
  return `${proto} · ${status}\n${url}`;
}
  ...
  <div ref="inspectorEl" class="inspector">
    <div class="sidebar" :class="{ collapsed: sidebarCollapsed }">
      <div class="sidebar-header">
        ...
        <template v-if="!sidebarCollapsed">
          ...
          <div class="sidebar-actions">
            <button class="toolbar-btn" title="Import .moqtrace" @click="handleImportClick">
              <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
                <path d="M3 1h6l4 4v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1zm6 1H3v12h9V5.5L9 2zM8 7l3 3h-2v3H7v-3H5l3-3z"/>
              </svg>
            </button>
      ...
      <template v-else>
        <div class="compact-list">
          ...
          >
            <span class="compact-dot" :style="{ background: statusColor(s) }"></span>
            <span class="compact-label">{{ compactHost(s.url) }}</span>
            <span v-if="compactBitrate(s)" class="compact-bitrate">{{ compactBitrate(s) }}</span>
          </div>
        </div>
    ...
    <div class="main">
      <div v-if="midSessionOpen" class="mid-session-banner">
        ...
        <span>
          Monitoring active — new connections will appear automatically.
          To capture connections established before DevTools opened,
      ...
      <div v-if="cspBlockedWorkers.length > 0" class="csp-warning-banner">
        <span class="banner-icon">
          <svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
            <path d="M8.9 1.5l6.6 12A1 1 0 0 1 14.6 15H1.4a1 1 0 0 1-.9-1.5l6.6-12a1 1 0 0 1 1.8 0zM7.5 10h1V6h-1v4zm0 2h1v-1h-1v1z"/>
          </svg>
        </span>
        <span>
