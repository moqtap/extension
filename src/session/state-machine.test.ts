...
import type { Draft14ClientSetup, Draft14ServerSetup, Draft14GoAway, Draft14Subscribe } from '@moqtap/codec/draft14';
...
describe('Draft-14 setup handshake', () => {
  ...
  it('full handshake produces session-ready side effect', () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      const readyEffect = result.sideEffects.find((e) => e.type === 'session-ready');
...
describe('GOAWAY transitions to draining', () => {
  ...
  it('GOAWAY produces session-draining side effect', () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      const drainingEffect = result.sideEffects.find((e) => e.type === 'session-draining');
