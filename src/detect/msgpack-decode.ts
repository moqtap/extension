...
function decode(data: Uint8Array, offset: number): [unknown, number] {
  ...
  switch (b) {
    case 0xc0: return [null, offset];       // nil
    case 0xc1: throw new Error('never used'); // never used
    case 0xc2: return [false, offset];      // false
    case 0xc3: return [true, offset];       // true

    // bin 8/16/32
    case 0xc4: return decodeBin(data, offset, data[offset++]);
    case 0xc5: { const n = readUint16(data, offset); return decodeBin(data, offset + 2, n); }
    case 0xc6: { const n = readUint32(data, offset); return decodeBin(data, offset + 4, n); }

    // ext 8/16/32
    case 0xc7: { const n = data[offset++]; return decodeExt(data, offset, n); }
    case 0xc8: { const n = readUint16(data, offset); return decodeExt(data, offset + 2, n); }
    case 0xc9: { const n = readUint32(data, offset); return decodeExt(data, offset + 4, n); }
    ...
    // uint 8/16/32/64
    case 0xcc: return [data[offset++], offset];
    case 0xcd: return [readUint16(data, offset), offset + 2];
    case 0xce: return [readUint32(data, offset), offset + 4];
    case 0xcf: {
      const view = new DataView(data.buffer, data.byteOffset + offset, 8);
    ...
    // int 8/16/32/64
    case 0xd0: return [data[offset] > 127 ? data[offset++] - 256 : data[offset++], offset];
    case 0xd1: {
    ...
    // fixext 1/2/4/8/16
    case 0xd4: return decodeExt(data, offset, 1);
    case 0xd5: return decodeExt(data, offset, 2);
    case 0xd6: return decodeExt(data, offset, 4);
    case 0xd7: return decodeExt(data, offset, 8);
    case 0xd8: return decodeExt(data, offset, 16);

    // str 8/16/32
    case 0xd9: return decodeStr(data, offset + 1, data[offset]);
    case 0xda: { const n = readUint16(data, offset); return decodeStr(data, offset + 2, n); }
    case 0xdb: { const n = readUint32(data, offset); return decodeStr(data, offset + 4, n); }

    // array 16/32
    case 0xdc: return decodeArray(data, offset + 2, readUint16(data, offset));
    case 0xdd: return decodeArray(data, offset + 4, readUint32(data, offset));

    // map 16/32
    case 0xde: return decodeMap(data, offset + 2, readUint16(data, offset));
    case 0xdf: return decodeMap(data, offset + 4, readUint32(data, offset));
...
function decodeStr(data: Uint8Array, offset: number, len: number): [string, number] {
  const s = TEXT_DECODER.decode(data.subarray(offset, offset + len));
  return [s, offset + len];
}

function decodeBin(data: Uint8Array, offset: number, len: number): [string, number] {
  const bytes = data.slice(offset, offset + len);
  return [Array.from(bytes, (b) => b.toString(16).padStart(2, '0')).join(''), offset + len];
}

function decodeExt(data: Uint8Array, offset: number, len: number): [unknown, number] {
...
function decodeArray(data: Uint8Array, offset: number, count: number): [unknown[], number] {
  const arr: unknown[] = [];
...
function decodeMap(data: Uint8Array, offset: number, count: number): [Record<string, unknown>, number] {
