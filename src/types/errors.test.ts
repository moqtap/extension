...
describe('Session Termination Error Codes (Table 8)', () => {
  ...
  for (const [name, code] of expected) {
    it(`${name} = 0x${code.toString(16)}`, () => {
      expect(SessionTerminationError[name as keyof typeof SessionTerminationError]).toBe(code);
    });
  }

  it('has exactly 21 error codes', () => {
    const values = Object.values(SessionTerminationError).filter((v) => typeof v === 'number');
...
describe('SUBSCRIBE_ERROR Codes (Table 9)', () => {
  ...
  for (const [name, code] of expected) {
    it(`${name} = 0x${code.toString(16)}`, () => {
      expect(SubscribeErrorCode[name as keyof typeof SubscribeErrorCode]).toBe(code);
    });
  }

  it('has exactly 8 error codes', () => {
    const values = Object.values(SubscribeErrorCode).filter((v) => typeof v === 'number');
...
describe('PUBLISH_DONE Codes (Table 10)', () => {
  ...
  it('has exactly 8 status codes', () => {
    const values = Object.values(PublishDoneCode).filter((v) => typeof v === 'number');
    expect(values).toHaveLength(8);
  });
});
...
describe('PUBLISH_ERROR Codes (Table 11)', () => {
  ...
  it('has exactly 5 error codes', () => {
    const values = Object.values(PublishErrorCode).filter((v) => typeof v === 'number');
...
describe('FETCH_ERROR Codes (Table 12)', () => {
  ...
  it('has exactly 12 error codes', () => {
    const values = Object.values(FetchErrorCode).filter((v) => typeof v === 'number');
...
describe('ANNOUNCE_ERROR Codes (Table 13)', () => {
  ...
  for (const [name, code] of expected) {
    it(`${name} = 0x${code.toString(16)}`, () => {
      expect(AnnounceErrorCode[name as keyof typeof AnnounceErrorCode]).toBe(code);
    });
  }

  it('has exactly 7 error codes', () => {
    const values = Object.values(AnnounceErrorCode).filter((v) => typeof v === 'number');
...
describe('SUBSCRIBE_NAMESPACE_ERROR Codes (Table 14)', () => {
  ...
  for (const [name, code] of expected) {
    it(`${name} = 0x${code.toString(16)}`, () => {
      expect(SubscribeNamespaceErrorCode[name as keyof typeof SubscribeNamespaceErrorCode]).toBe(code);
    });
  }

  it('has exactly 8 error codes', () => {
    const values = Object.values(SubscribeNamespaceErrorCode).filter((v) => typeof v === 'number');
    expect(values).toHaveLength(8);
  });
});
...
describe('Data Stream Reset Error Codes (Table 15)', () => {
  ...
  for (const [name, code] of expected) {
    it(`${name} = 0x${code.toString(16)}`, () => {
      expect(DataStreamResetError[name as keyof typeof DataStreamResetError]).toBe(code);
    });
  }

  it('has exactly 4 error codes', () => {
    const values = Object.values(DataStreamResetError).filter((v) => typeof v === 'number');
