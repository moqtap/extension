...
import { installWebTransportHook, uninstallWebTransportHook } from './webtransport-hook';
...
function createMockGlobal(): typeof globalThis & { WebTransport: typeof MockWebTransport } {
  return {
...
describe('WebTransport hook — installation', () => {
  ...
  it('returns a cleanup function that restores the original constructor', () => {
    const mockGlobal = createMockGlobal();
    const originalWT = mockGlobal.WebTransport;
    const cleanup = installWebTransportHook(
      mockGlobal, vi.fn(),
...
describe('WebTransport hook — session capture', () => {
  it('captures new WebTransport session with URL', () => {
    ...
    const sessions: InterceptedSession[] = [];
    installWebTransportHook(
      mockGlobal, (s) => sessions.push(s),
      { onData: vi.fn(), onClose: vi.fn(), onError: vi.fn() },
    );

    const _wt = new (mockGlobal.WebTransport as any)('https://relay.example.com/moq');
...
describe('WebTransport hook — transparency', () => {
  it('intercepted WebTransport still exposes .ready', async () => {
    ...
    const wt = new (mockGlobal.WebTransport as any)('https://relay.example.com/moq');
